...
let map_semicolon (env : env) (v1 : CST.semicolon) =
  (* ";" *) token env v1
...
and map_assignable_selector (env : env) (x : CST.assignable_selector) =
  ...
  | `Cond_assi_sele (v1, v2) -> R.Case ("Cond_assi_sele",
      let v1 = (* "?." *) token env v1 in
      let v2 = (* pattern [a-zA-Z_$][\w$]* *) token env v2 in
      R.Tuple [v1; v2]
    )
...
and map_cascade_selector (env : env) (x : CST.cascade_selector) =
  (match x with
  | `Opt_null_type_LBRACK_exp_RBRACK (v1, v2, v3, v4) -> R.Case ("Opt_null_type_LBRACK_exp_RBRACK",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "?" *) token env tok
...
and map_constructor_invocation (env : env) ((v1, v2, v3, v4, v5) : CST.constructor_invocation) =
...
and map_default_named_parameter (env : env) (x : CST.default_named_parameter) =
  (match x with
  | `Opt_requ_formal_param_opt_EQ_exp (v1, v2, v3) -> R.Case ("Opt_requ_formal_param_opt_EQ_exp",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
  ...
  | `Opt_requ_formal_param_opt_COLON_exp (v1, v2, v3) -> R.Case ("Opt_requ_formal_param_opt_COLON_exp",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "required" *) token env tok
...
and map_element (env : env) (x : CST.element) =
  ...
  | `If_elem (v1, v2, v3, v4) -> R.Case ("If_elem",
  ...
  | `For_elem (v1, v2, v3, v4, v5, v6) -> R.Case ("For_elem",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "await" *) token env tok
...
and map_expression (env : env) (x : CST.expression) =
  (match x with
  | `Choice_assign_exp x -> R.Case ("Choice_assign_exp",
      (match x with
...
and map_local_variable_declaration (env : env) ((v1, v2) : CST.local_variable_declaration) =
...
and map_primary (env : env) (x : CST.primary) =
  ...
  | `Const_obj_exp (v1, v2, v3, v4) -> R.Case ("Const_obj_exp",
      let v1 = (* const_builtin *) token env v1 in
      let v2 = map_type_not_void env v2 in
      let v3 =
        (match v3 with
  ...
  | `LPAR_exp_RPAR x -> R.Case ("LPAR_exp_RPAR",
...
and map_switch_block (env : env) ((v1, v2, v3) : CST.switch_block) =
  ...
  let v2 =
    R.List (List.map (fun x ->
      (match x with
      | `Switch_label x -> R.Case ("Switch_label",
          map_switch_label env x
        )
      | `Stmt x -> R.Case ("Stmt",
          map_statement env x
        )
...
and map_switch_label (env : env) ((v1, v2) : CST.switch_label) =
  ...
  let v2 =
    (match v2 with
    | `Case_buil_exp_COLON (v1, v2, v3) -> R.Case ("Case_buil_exp_COLON",
...
let map_constant_constructor_signature (env : env) ((v1, v2, v3) : CST.constant_constructor_signature) =
  let v1 = (* const_builtin *) token env v1 in
...
let map_part_of_directive (env : env) ((v1, v2, v3, v4, v5) : CST.part_of_directive) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_metadata env x
...
let map_type_alias (env : env) (x : CST.type_alias) =
  (match x with
  | `Type_type_name_opt_type_params_EQ_func_type_SEMI (v1, v2, v3, v4, v5, v6) -> R.Case ("Type_type_name_opt_type_params_EQ_func_type_SEMI",
  ...
  | `Type_opt_type_type_name_formal_param_part_SEMI (v1, v2, v3, v4, v5) -> R.Case ("Type_opt_type_type_name_formal_param_part_SEMI",
      let v1 = (* "typedef" *) token env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
...
let map_configuration_uri (env : env) ((v1, v2, v3, v4, v5) : CST.configuration_uri) =
  let v1 = (* "if" *) token env v1 in
  let v2 = (* "(" *) token env v2 in
...
let map_initializer_list_entry (env : env) (x : CST.initializer_list_entry) =
  (match x with
  | `Super_opt_DOT_qual_args (v1, v2, v3) -> R.Case ("Super_opt_DOT_qual_args",
      let v1 = (* "super" *) token env v1 in
      let v2 =
        (match v2 with
        | Some (v1, v2) -> R.Option (Some (
...
let map_extension_declaration (env : env) (x : CST.extension_declaration) =
  (match x with
  | `Exte_opt_id_opt_type_params_on_type_exte_body (v1, v2, v3, v4, v5, v6) -> R.Case ("Exte_opt_id_opt_type_params_on_type_exte_body",
...
let map_class_definition (env : env) (x : CST.class_definition) =
  (match x with
  | `Opt_abst_class_id_opt_type_params_opt_supe_opt_inters_class_body (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("Opt_abst_class_id_opt_type_params_opt_supe_opt_inters_class_body",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
  ...
  | `Opt_meta_opt_abst_class_mixin_app_class (v1, v2, v3, v4) -> R.Case ("Opt_meta_opt_abst_class_mixin_app_class",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
...
let map_top_level_definition (env : env) (x : CST.top_level_definition) =
  ...
  | `Mixin_decl (v1, v2, v3, v4, v5, v6) -> R.Case ("Mixin_decl",
  ...
  | `Opt_exte_buil_func_sign_semi (v1, v2, v3) -> R.Case ("Opt_exte_buil_func_sign_semi",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "external" *) token env tok
  ...
  | `Opt_exte_buil_getter_sign_semi (v1, v2, v3) -> R.Case ("Opt_exte_buil_getter_sign_semi",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "external" *) token env tok
  ...
  | `Opt_exte_buil_setter_sign_semi (v1, v2, v3) -> R.Case ("Opt_exte_buil_setter_sign_semi",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "external" *) token env tok
  ...
  | `Func_sign_func_body x -> R.Case ("Func_sign_func_body",
      map_lambda_expression env x
    )
  | `Getter_sign_func_body (v1, v2) -> R.Case ("Getter_sign_func_body",
      let v1 = map_getter_signature env v1 in
      let v2 = map_function_body env v2 in
      R.Tuple [v1; v2]
    )
  | `Setter_sign_func_body (v1, v2) -> R.Case ("Setter_sign_func_body",
      let v1 = map_setter_signature env v1 in
      let v2 = map_function_body env v2 in
      R.Tuple [v1; v2]
    )
  | `Choice_final_buil_opt_type_static_final_decl_list_semi (v1, v2, v3, v4) -> R.Case ("Choice_final_buil_opt_type_static_final_decl_list_semi",
      let v1 = map_final_or_const env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
  ...
  | `Late_buil_final_buil_opt_type_init_id_list_semi (v1, v2, v3, v4, v5) -> R.Case ("Late_buil_final_buil_opt_type_init_id_list_semi",
      let v1 = (* "late" *) token env v1 in
      let v2 = (* final_builtin *) token env v2 in
      let v3 =
        (match v3 with
  ...
  | `Opt_late_buil_choice_type_init_id_list_semi (v1, v2, v3, v4) -> R.Case ("Opt_late_buil_choice_type_init_id_list_semi",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "late" *) token env tok
...
let map_program (env : env) (x : CST.program) =
  (match x with
  | `Opt_script_tag_opt_libr_name_rep_import_or_export_rep_part_dire_rep_part_of_dire_rep_opt_meta_top_level_defi_rep_stmt (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("Opt_script_tag_opt_libr_name_rep_import_or_export_rep_part_dire_rep_part_of_dire_rep_opt_meta_top_level_defi_rep_stmt",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
