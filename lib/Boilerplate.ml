...
let map_identifier_dollar_escaped (env : env) (tok : CST.identifier_dollar_escaped) =
  (* pattern ([a-zA-Z_]|(\\\$))([\w]|(\\\$))* *) token env tok
...
let map_tok_is (env : env) (tok : CST.tok_is) =
  (* tok_is *) token env tok
...
let map_assignment_operator (env : env) (x : CST.assignment_operator) =
  ...
  | `TILDESLASHEQ tok -> R.Case ("TILDESLASHEQ",
      (* "~/=" *) token env tok
    )
...
let map_documentation_block_comment (env : env) (tok : CST.documentation_block_comment) =
...
let map_multiplicative_operator_ (env : env) (x : CST.multiplicative_operator_) =
  ...
  | `TILDESLASH tok -> R.Case ("TILDESLASH",
      (* "~/" *) token env tok
    )
  )
...
let map_part_of_builtin (env : env) (tok : CST.part_of_builtin) =
  (* part_of_builtin *) token env tok

let map_rethrow_builtin (env : env) (tok : CST.rethrow_builtin) =
  (* rethrow_builtin *) token env tok
...
let map_anon_choice_async_725f72f (env : env) (x : CST.anon_choice_async_725f72f) =
  (match x with
  | `Async tok -> R.Case ("Async",
      (* "async" *) token env tok
    )
  | `Asyn tok -> R.Case ("Asyn",
      (* "async*" *) token env tok
    )
...
let map_template_chars_raw_slash (env : env) (tok : CST.template_chars_raw_slash) =
  (* template_chars_raw_slash *) token env tok

let map_template_chars_single_single (env : env) (tok : CST.template_chars_single_single) =
  (* template_chars_single_single *) token env tok

let map_template_chars_double_single (env : env) (tok : CST.template_chars_double_single) =
...
let map_identifier_or_new (env : env) (x : CST.identifier_or_new) =
  (match x with
  | `Id tok -> R.Case ("Id",
      (* pattern [a-zA-Z_$][\w$]* *) token env tok
    )
  | `New_buil tok -> R.Case ("New_buil",
      (* "new" *) token env tok
    )
  )
...
let map_final_or_const (env : env) (x : CST.final_or_const) =
  (match x with
  | `Final_buil tok -> R.Case ("Final_buil",
      (* final_builtin *) token env tok
    )
  | `Const_buil tok -> R.Case ("Const_buil",
      (* const_builtin *) token env tok
    )
  )

let map_numeric_literal (env : env) (x : CST.numeric_literal) =
  (match x with
...
let map_anon_choice_final_buil_80e16d2 (env : env) (x : CST.anon_choice_final_buil_80e16d2) =
  (match x with
  | `Final_buil tok -> R.Case ("Final_buil",
      (* final_builtin *) token env tok
    )
  | `Infe_type tok -> R.Case ("Infe_type",
      (* "var" *) token env tok
    )
  )

let map_class_modifiers (env : env) ((v1, v2) : CST.class_modifiers) =
  let v1 =
    (match v1 with
    | `Sealed tok -> R.Case ("Sealed",
        (* "sealed" *) token env tok
      )
    | `Opt_abst_opt_choice_base (v1, v2) -> R.Case ("Opt_abst_opt_choice_base",
        let v1 =
          (match v1 with
...
let map_mixin_class_modifiers (env : env) ((v1, v2, v3, v4) : CST.mixin_class_modifiers) =
  let v1 =
    (match v1 with
    | Some tok -> R.Option (Some (
        (* "abstract" *) token env tok
...
let map_type_name (env : env) ((v1, v2) : CST.type_name) =
  ...
  let v2 =
    (match v2 with
    | Some x -> R.Option (Some (
        map_type_dot_identifier env x
      ))
    | None -> R.Option None)
  in
  R.Tuple [v1; v2]

let map_dotted_identifier_list (env : env) ((v1, v2) : CST.dotted_identifier_list) =
  let v1 = (* pattern [a-zA-Z_$][\w$]* *) token env v1 in
...
let map_prefix_operator (env : env) (x : CST.prefix_operator) =
  (match x with
  | `Minus_op tok -> R.Case ("Minus_op",
      (* "-" *) token env tok
    )
  | `Nega_op tok -> R.Case ("Nega_op",
      (* "!" *) token env tok
    )
...
let map_raw_string_literal_single_quotes (env : env) ((v1, v2, v3) : CST.raw_string_literal_single_quotes) =
  let v1 = (* "r'" *) token env v1 in
  let v2 =
    R.List (List.map (fun x ->
      (match x with
      | `Temp_chars_single_single tok -> R.Case ("Temp_chars_single_single",
          (* template_chars_single_single *) token env tok
        )
      | `DQUOT tok -> R.Case ("DQUOT",
          (* "\"" *) token env tok
        )
      | `Temp_chars_raw_slash tok -> R.Case ("Temp_chars_raw_slash",
          (* template_chars_raw_slash *) token env tok
        )
      | `Unused_esc_seq tok -> R.Case ("Unused_esc_seq",
          (* unused_escape_sequence *) token env tok
        )
      | `Sub_str_test x -> R.Case ("Sub_str_test",
          map_sub_string_test env x
        )
...
let map_raw_string_literal_double_quotes_multiple (env : env) ((v1, v2, v3) : CST.raw_string_literal_double_quotes_multiple) =
  let v1 = (* "r\"\"\"" *) token env v1 in
  let v2 =
    R.List (List.map (fun x ->
      (match x with
      | `Temp_chars_double tok -> R.Case ("Temp_chars_double",
          (* template_chars_double *) token env tok
        )
...
let map_raw_string_literal_single_quotes_multiple (env : env) ((v1, v2, v3) : CST.raw_string_literal_single_quotes_multiple) =
  let v1 = (* "r'''" *) token env v1 in
  let v2 =
    R.List (List.map (fun x ->
      (match x with
      | `Temp_chars_single tok -> R.Case ("Temp_chars_single",
          (* template_chars_single *) token env tok
        )
      | `DQUOT tok -> R.Case ("DQUOT",
          (* "\"" *) token env tok
...
let map_raw_string_literal_double_quotes (env : env) ((v1, v2, v3) : CST.raw_string_literal_double_quotes) =
  let v1 = (* "r\"" *) token env v1 in
  let v2 =
    R.List (List.map (fun x ->
      (match x with
      | `Temp_chars_double_single tok -> R.Case ("Temp_chars_double_single",
          (* template_chars_double_single *) token env tok
        )
      | `SQUOT tok -> R.Case ("SQUOT",
          (* "'" *) token env tok
...
let map_qualified (env : env) (x : CST.qualified) =
  (match x with
  | `Type_name_DOT_id_or_new (v1, v2, v3) -> R.Case ("Type_name_DOT_id_or_new",
      let v1 = map_type_name env v1 in
      let v2 = (* "." *) token env v2 in
      let v3 = map_identifier_or_new env v3 in
      R.Tuple [v1; v2; v3]
    )
  | `Type_name_DOT_type_name_DOT_id_or_new (v1, v2, v3, v4, v5) -> R.Case ("Type_name_DOT_type_name_DOT_id_or_new",
      let v1 = map_type_name env v1 in
      let v2 = (* "." *) token env v2 in
      let v3 = map_type_name env v3 in
      let v4 = (* "." *) token env v4 in
      let v5 = map_identifier_or_new env v5 in
...
let rec map_additive_expression (env : env) (x : CST.additive_expression) =
  (match x with
  | `Real_exp_rep1_addi_op_real_exp (v1, v2) -> R.Case ("Real_exp_rep1_addi_op_real_exp",
  ...
  | `Super_rep1_addi_op_real_exp (v1, v2) -> R.Case ("Super_rep1_addi_op_real_exp",
...
and map_annotation (env : env) ((v1, v2, v3) : CST.annotation) =
  ...
  let v3 =
    (match v3 with
    | `Opt_type_args_args opt -> R.Case ("Opt_type_args_args",
        (match opt with
        | Some (v1, v2) -> R.Option (Some (
...
and map_any_argument (env : env) (x : CST.any_argument) =
  (match x with
  | `Arg x -> R.Case ("Arg",
      map_argument env x
    )
  | `Named_arg x -> R.Case ("Named_arg",
...
and map_argument_list (env : env) ((v1, v2, v3) : CST.argument_list) =
  let v1 = map_any_argument env v1 in
  let v2 =
    R.List (List.map (fun (v1, v2) ->
      let v1 = (* "," *) token env v1 in
      let v2 = map_any_argument env v2 in
      R.Tuple [v1; v2]
    ) v2)
  in
  let v3 =
...
and map_assertion (env : env) ((v1, v2) : CST.assertion) =
  let v1 = (* assert_builtin *) token env v1 in
  let v2 = map_assertion_arguments env v2 in
  R.Tuple [v1; v2]

and map_assertion_arguments (env : env) ((v1, v2, v3, v4, v5) : CST.assertion_arguments) =
  let v1 = (* "(" *) token env v1 in
  let v2 = map_argument env v2 in
  let v3 =
    (match v3 with
...
and map_assignable_expression (env : env) (x : CST.assignable_expression) =
  (match x with
  | `Prim_assi_sele_part (v1, v2) -> R.Case ("Prim_assi_sele_part",
      let v1 = map_primary env v1 in
      let v2 = map_assignable_selector_part env v2 in
      R.Tuple [v1; v2]
    )
  | `Super_unco_assi_sele (v1, v2) -> R.Case ("Super_unco_assi_sele",
      let v1 = (* "super" *) token env v1 in
      let v2 = map_unconditional_assignable_selector env v2 in
      R.Tuple [v1; v2]
    )
  | `Cons_invo_assi_sele_part (v1, v2) -> R.Case ("Cons_invo_assi_sele_part",
...
and map_assignable_selector (env : env) (x : CST.assignable_selector) =
  (match x with
  | `Unco_assi_sele x -> R.Case ("Unco_assi_sele",
      map_unconditional_assignable_selector env x
    )
  | `Cond_assi_sele x -> R.Case ("Cond_assi_sele",
      map_conditional_assignable_selector env x
    )
  )

and map_assignable_selector_part (env : env) ((v1, v2) : CST.assignable_selector_part) =
...
and map_bitwise_and_expression (env : env) (x : CST.bitwise_and_expression) =
  (match x with
  | `Real_exp_rep1_AMP_real_exp (v1, v2) -> R.Case ("Real_exp_rep1_AMP_real_exp",
      let v1 = map_real_expression env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "&" *) token env v1 in
  ...
  | `Super_rep1_AMP_real_exp (v1, v2) -> R.Case ("Super_rep1_AMP_real_exp",
      let v1 = (* "super" *) token env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "&" *) token env v1 in
...
and map_bitwise_or_expression (env : env) (x : CST.bitwise_or_expression) =
  (match x with
  | `Real_exp_rep1_BAR_real_exp (v1, v2) -> R.Case ("Real_exp_rep1_BAR_real_exp",
      let v1 = map_real_expression env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "|" *) token env v1 in
  ...
  | `Super_rep1_BAR_real_exp (v1, v2) -> R.Case ("Super_rep1_BAR_real_exp",
      let v1 = (* "super" *) token env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "|" *) token env v1 in
...
and map_bitwise_xor_expression (env : env) (x : CST.bitwise_xor_expression) =
  (match x with
  | `Real_exp_rep1_HAT_real_exp (v1, v2) -> R.Case ("Real_exp_rep1_HAT_real_exp",
      let v1 = map_real_expression env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "^" *) token env v1 in
  ...
  | `Super_rep1_HAT_real_exp (v1, v2) -> R.Case ("Super_rep1_HAT_real_exp",
      let v1 = (* "super" *) token env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "^" *) token env v1 in
...
and map_cascade_assignment_section (env : env) ((v1, v2) : CST.cascade_assignment_section) =
...
and map_cascade_section (env : env) ((v1, v2, v3, v4, v5) : CST.cascade_section) =
  let v1 =
    ...
    | `QMARKDOTDOT tok -> R.Case ("QMARKDOTDOT",
        (* "?.." *) token env tok
      )
    )
  in
  ...
  let v5 =
    (match v5 with
    | Some x -> R.Option (Some (
        map_cascade_assignment_section env x
      ))
    | None -> R.Option None)
  in
  R.Tuple [v1; v2; v3; v4; v5]
...
and map_cascade_selector (env : env) (x : CST.cascade_selector) =
  (match x with
  | `Opt_null_sele_index_sele (v1, v2) -> R.Case ("Opt_null_sele_index_sele",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "?" *) token env tok
        ...
        | None -> R.Option None)
      in
      let v2 = map_index_selector env v2 in
      R.Tuple [v1; v2]
    )
  | `Id tok -> R.Case ("Id",
      (* pattern [a-zA-Z_$][\w$]* *) token env tok
...
and map_conditional_assignable_selector (env : env) (x : CST.conditional_assignable_selector) =
  (match x with
  | `QMARKDOT_id (v1, v2) -> R.Case ("QMARKDOT_id",
      let v1 = (* "?." *) token env v1 in
      let v2 = (* pattern [a-zA-Z_$][\w$]* *) token env v2 in
      R.Tuple [v1; v2]
    )
  | `QMARK_index_sele (v1, v2) -> R.Case ("QMARK_index_sele",
      let v1 = (* "?" *) token env v1 in
      let v2 = map_index_selector env v2 in
      R.Tuple [v1; v2]
    )
...
and map_constant_pattern (env : env) (x : CST.constant_pattern) =
  ...
  | `Opt_minus_op_nume_lit (v1, v2) -> R.Case ("Opt_minus_op_nume_lit",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "-" *) token env tok
        ...
        | None -> R.Option None)
      in
      let v2 = map_numeric_literal env v2 in
      R.Tuple [v1; v2]
    )
  | `Str_lit x -> R.Case ("Str_lit",
      map_uri env x
    )
  | `Symb_lit x -> R.Case ("Symb_lit",
      map_symbol_literal env x
    )
  | `Id tok -> R.Case ("Id",
      (* pattern [a-zA-Z_$][\w$]* *) token env tok
    )
  ...
  | `Const_buil_opt_type_args_LCURL_elem_rep_COMMA_elem_opt_COMMA_RCURL (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("Const_buil_opt_type_args_LCURL_elem_rep_COMMA_elem_opt_COMMA_RCURL",
      let v1 = (* const_builtin *) token env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
  ...
  | `Const_buil_LPAR_exp_RPAR (v1, v2, v3, v4) -> R.Case ("Const_buil_LPAR_exp_RPAR",
      let v1 = (* const_builtin *) token env v1 in
      let v2 = (* "(" *) token env v2 in
      let v3 = map_argument env v3 in
      let v4 = (* ")" *) token env v4 in
...
and map_constructor_invocation (env : env) (x : CST.constructor_invocation) =
  (match x with
  | `Type_name_type_args_DOT_id_args (v1, v2, v3, v4, v5) -> R.Case ("Type_name_type_args_DOT_id_args",
      let v1 = map_type_name env v1 in
  ...
  | `Type_name_DOT_new_buil_args (v1, v2, v3, v4) -> R.Case ("Type_name_DOT_new_buil_args",
      let v1 = map_type_name env v1 in
      let v2 = (* "." *) token env v2 in
      let v3 = (* "new" *) token env v3 in
      let v4 = map_arguments env v4 in
      R.Tuple [v1; v2; v3; v4]
    )
  )

and map_constructor_param (env : env) ((v1, v2, v3, v4, v5) : CST.constructor_param) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_final_const_var_or_type env x
      ))
    | None -> R.Option None)
  in
  ...
  let v5 =
    (match v5 with
    | Some x -> R.Option (Some (
        map_formal_parameter_part env x
      ))
    | None -> R.Option None)
  in
  R.Tuple [v1; v2; v3; v4; v5]

and map_declared_identifier (env : env) ((v1, v2, v3, v4) : CST.declared_identifier) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_metadata env x
...
and map_default_named_parameter (env : env) (x : CST.default_named_parameter) =
  (match x with
  | `Opt_meta_opt_requ_formal_param_opt_EQ_exp (v1, v2, v3, v4) -> R.Case ("Opt_meta_opt_requ_formal_param_opt_EQ_exp",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
  ...
  | `Opt_meta_opt_requ_formal_param_opt_COLON_exp (v1, v2, v3, v4) -> R.Case ("Opt_meta_opt_requ_formal_param_opt_COLON_exp",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
...
and map_element (env : env) (x : CST.element) =
  ...
  | `If_elem (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("If_elem",
      ...
      let v4 =
        ...
        | Some (v1, v2) -> R.Option (Some (
            ...
            let v2 = map_guarded_pattern env v2 in
            R.Tuple [v1; v2]
          ))
        | None -> R.Option None)
      in
  ...
  | `For_elem (v1, v2, v3, v4) -> R.Case ("For_elem",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "await" *) token env tok
      ...
      let v3 = map_for_loop_parts env v3 in
      let v4 = map_element env v4 in
      R.Tuple [v1; v2; v3; v4]
    )
  )
...
and map_equality_expression (env : env) (x : CST.equality_expression) =
  (match x with
  | `Real_exp_equa_op_real_exp (v1, v2, v3) -> R.Case ("Real_exp_equa_op_real_exp",
      let v1 = map_real_expression env v1 in
      let v2 = (* equality_operator *) token env v2 in
      let v3 = map_real_expression env v3 in
      R.Tuple [v1; v2; v3]
    )
  | `Super_equa_op_real_exp (v1, v2, v3) -> R.Case ("Super_equa_op_real_exp",
      let v1 = (* "super" *) token env v1 in
      let v2 = (* equality_operator *) token env v2 in
      let v3 = map_real_expression env v3 in
      R.Tuple [v1; v2; v3]
...
and map_expression (env : env) (x : CST.expression) =
  (match x with
  | `Choice_pat_assign x -> R.Case ("Choice_pat_assign",
      (match x with
      | `Pat_assign x -> R.Case ("Pat_assign",
          map_pattern_assignment env x
        )
      | `Assign_exp x -> R.Case ("Assign_exp",
          map_assignment_expression env x
        )
      | `Throw_exp x -> R.Case ("Throw_exp",
          map_throw_expression env x
        )
      | `Reth_exp tok -> R.Case ("Reth_exp",
          (* rethrow_builtin *) token env tok
        )
      | `Real_exp_rep_casc_sect (v1, v2) -> R.Case ("Real_exp_rep_casc_sect",
          let v1 = map_real_expression env v1 in
...
and map_expression_statement (env : env) (x : CST.expression_statement) =
  ...
  | `Exp_semi (v1, v2) -> R.Case ("Exp_semi",
      let v1 = map_argument env v1 in
      let v2 = (* semicolon *) token env v2 in
      R.Tuple [v1; v2]
    )
  | `Semg_ellips tok -> R.Case ("Semg_ellips",
      (* "..." *) token env tok
...
and map_expression_without_cascade (env : env) (x : CST.expression_without_cascade) =
  (match x with
  | `Assign_exp_with_casc (v1, v2, v3) -> R.Case ("Assign_exp_with_casc",
      let v1 = map_assignable_expression env v1 in
      let v2 = map_assignment_operator env v2 in
      let v3 = map_expression_without_cascade env v3 in
      R.Tuple [v1; v2; v3]
    )
  | `Real_exp x -> R.Case ("Real_exp",
      map_real_expression env x
    )
  | `Throw_exp_with_casc (v1, v2) -> R.Case ("Throw_exp_with_casc",
...
and map_final_const_var_or_type (env : env) (x : CST.final_const_var_or_type) =
  (match x with
  | `Opt_late_buil_final_buil_opt_type (v1, v2, v3) -> R.Case ("Opt_late_buil_final_buil_opt_type",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "late" *) token env tok
  ...
  | `Const_buil_opt_type (v1, v2) -> R.Case ("Const_buil_opt_type",
      let v1 = (* const_builtin *) token env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
  ...
  | `Opt_late_buil_var_or_type (v1, v2) -> R.Case ("Opt_late_buil_var_or_type",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "late" *) token env tok
...
and map_final_var_or_type (env : env) (x : CST.final_var_or_type) =
  (match x with
  | `Infe_type tok -> R.Case ("Infe_type",
      (* "var" *) token env tok
    )
  ...
  | `Opt_final_buil_type (v1, v2) -> R.Case ("Opt_final_buil_type",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* final_builtin *) token env tok
...
and map_for_loop_parts (env : env) ((v1, v2, v3) : CST.for_loop_parts) =
  let v1 = (* "(" *) token env v1 in
  let v2 = map_for_loop_parts_ env v2 in
  let v3 = (* ")" *) token env v3 in
  R.Tuple [v1; v2; v3]

and map_for_loop_parts_ (env : env) (x : CST.for_loop_parts_) =
  (match x with
  | `Choice_decl_id_in_exp (v1, v2, v3) -> R.Case ("Choice_decl_id_in_exp",
      let v1 =
        (match v1 with
        | `Decl_id x -> R.Case ("Decl_id",
            map_declared_identifier env x
          )
        | `Id tok -> R.Case ("Id",
            (* pattern [a-zA-Z_$][\w$]* *) token env tok
          )
  ...
  | `Choice_final_buil_outer_pat_in_exp (v1, v2, v3, v4) -> R.Case ("Choice_final_buil_outer_pat_in_exp",
      let v1 = map_anon_choice_final_buil_80e16d2 env v1 in
      let v2 = map_outer_pattern env v2 in
...
and map_formal_parameter (env : env) (x : CST.formal_parameter) =
  ...
  | `Normal_formal_param (v1, v2) -> R.Case ("Normal_formal_param",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
      ...
      let v2 =
        (match v2 with
        | `Func_formal_param x -> R.Case ("Func_formal_param",
            map_function_formal_parameter env x
          )
        | `Simple_formal_param x -> R.Case ("Simple_formal_param",
            map_simple_formal_parameter env x
          )
        | `Cons_param x -> R.Case ("Cons_param",
            map_constructor_param env x
          )
        | `Super_formal_param x -> R.Case ("Super_formal_param",
            map_super_formal_parameter env x
          )
        )
      in
      R.Tuple [v1; v2]
...
and map_function_body (env : env) (x : CST.function_body) =
  (match x with
  | `Opt_async_EQGT_exp_semi (v1, v2, v3, v4) -> R.Case ("Opt_async_EQGT_exp_semi",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "async" *) token env tok
  ...
  | `Opt_choice_async_blk (v1, v2) -> R.Case ("Opt_choice_async_blk",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_anon_choice_async_725f72f env x
          ))
        | None -> R.Option None)
      in
      let v2 = map_block env v2 in
...
and map_function_expression_body (env : env) (x : CST.function_expression_body) =
  (match x with
  | `Opt_async_EQGT_exp (v1, v2, v3) -> R.Case ("Opt_async_EQGT_exp",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "async" *) token env tok
...
and map_function_formal_parameter (env : env) ((v1, v2, v3, v4, v5) : CST.function_formal_parameter) =
  let v1 =
    (match v1 with
    | Some tok -> R.Option (Some (
        (* "covariant" *) token env tok
...
and map_function_signature (env : env) ((v1, v2, v3, v4) : CST.function_signature) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_type_ env x
...
and map_function_type (env : env) (x : CST.function_type) =
  (match x with
  | `Func_type_tails x -> R.Case ("Func_type_tails",
      map_function_type_tails env x
    )
  | `Type_not_func_func_type_tails (v1, v2) -> R.Case ("Type_not_func_func_type_tails",
...
and map_function_type_tail (env : env) ((v1, v2, v3, v4, v5) : CST.function_type_tail) =
...
and map_guarded_pattern (env : env) ((v1, v2) : CST.guarded_pattern) =
...
and map_index_selector (env : env) ((v1, v2, v3) : CST.index_selector) =
  let v1 = (* "[" *) token env v1 in
  let v2 = map_argument env v2 in
  let v3 = (* "]" *) token env v3 in
  R.Tuple [v1; v2; v3]
...
and map_initialized_variable_definition (env : env) ((v1, v2, v3) : CST.initialized_variable_definition) =
...
and map_list_pattern (env : env) ((v1, v2, v3, v4) : CST.list_pattern) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_type_arguments env x
  ...
  let v3 =
    ...
    | Some (v1, v2, v3) -> R.Option (Some (
        let v1 = map_list_pattern_element env v1 in
        let v2 =
          R.List (List.map (fun (v1, v2) ->
            let v1 = (* "," *) token env v1 in
            let v2 = map_list_pattern_element env v2 in
            R.Tuple [v1; v2]
          ) v2)
        in
        let v3 =
...
and map_list_pattern_element (env : env) (x : CST.list_pattern_element) =
  (match x with
  | `Pat x -> R.Case ("Pat",
      map_pattern env x
    )
  | `Rest_pat (v1, v2) -> R.Case ("Rest_pat",
      let v1 = (* "..." *) token env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
...
and map_literal (env : env) (x : CST.literal) =
  ...
  | `Set_or_map_lit (v1, v2, v3, v4, v5) -> R.Case ("Set_or_map_lit",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* const_builtin *) token env tok
      ...
      let v4 =
        (match v4 with
        | Some x -> R.Option (Some (
            map_anon_elem_rep_COMMA_elem_opt_COMMA_4ec364f env x
          ))
        | None -> R.Option None)
      in
      let v5 = (* "}" *) token env v5 in
  ...
  | `List_lit (v1, v2, v3, v4, v5) -> R.Case ("List_lit",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* const_builtin *) token env tok
  ...
  | `Record_lit (v1, v2) -> R.Case ("Record_lit",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* const_builtin *) token env tok
        ...
        | None -> R.Option None)
      in
      let v2 = map_record_literal_no_const env v2 in
      R.Tuple [v1; v2]
    )
  )

and map_local_variable_declaration (env : env) (x : CST.local_variable_declaration) =
  (match x with
  | `Opt_meta_init_var_defi_semi (v1, v2, v3) -> R.Case ("Opt_meta_init_var_defi_semi",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
        ...
        | None -> R.Option None)
      in
      let v2 = map_initialized_variable_definition env v2 in
      let v3 = (* semicolon *) token env v3 in
      R.Tuple [v1; v2; v3]
    )
  | `Opt_meta_pat_var_decl_semi (v1, v2, v3) -> R.Case ("Opt_meta_pat_var_decl_semi",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
        ...
        | None -> R.Option None)
      in
      let v2 = map_pattern_variable_declaration env v2 in
      let v3 = (* semicolon *) token env v3 in
      R.Tuple [v1; v2; v3]
    )
  )

and map_logical_and_pattern (env : env) ((v1, v2) : CST.logical_and_pattern) =
  let v1 = map_relational_pattern env v1 in
  let v2 =
    R.List (List.map (fun (v1, v2) ->
      let v1 = (* "&&" *) token env v1 in
      let v2 = map_relational_pattern env v2 in
      R.Tuple [v1; v2]
    ) v2)
  in
  R.Tuple [v1; v2]

and map_map_pattern (env : env) ((v1, v2, v3, v4) : CST.map_pattern) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_type_arguments env x
  ...
  let v3 =
    (match v3 with
    | Some (v1, v2, v3) -> R.Option (Some (
        let v1 = map_map_pattern_entry env v1 in
        let v2 =
          R.List (List.map (fun (v1, v2) ->
            let v1 = (* "," *) token env v1 in
            let v2 = map_map_pattern_entry env v2 in
            R.Tuple [v1; v2]
          ) v2)
        in
        let v3 =
...
and map_map_pattern_entry (env : env) (x : CST.map_pattern_entry) =
  (match x with
  | `Exp_COLON_pat (v1, v2, v3) -> R.Case ("Exp_COLON_pat",
      let v1 = map_argument env v1 in
      let v2 = (* ":" *) token env v2 in
...
and map_multiplicative_expression (env : env) (x : CST.multiplicative_expression) =
  (match x with
  | `Un_exp_rep1_mult_op_un_exp (v1, v2) -> R.Case ("Un_exp_rep1_mult_op_un_exp",
  ...
  | `Super_rep1_mult_op_un_exp (v1, v2) -> R.Case ("Super_rep1_mult_op_un_exp",
      let v1 = (* "super" *) token env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = map_multiplicative_operator env v1 in
...
and map_normal_formal_parameters (env : env) ((v1, v2) : CST.normal_formal_parameters) =
  let v1 = map_formal_parameter env v1 in
  let v2 =
    R.List (List.map (fun (v1, v2) ->
      let v1 = (* "," *) token env v1 in
...
and map_normal_parameter_type (env : env) ((v1, v2) : CST.normal_parameter_type) =
  ...
  let v2 =
    (match v2 with
    | `Typed_id x -> R.Case ("Typed_id",
        map_typed_identifier env x
      )
    | `Type x -> R.Case ("Type",
        map_type_ env x
      )
...
and map_object_pattern (env : env) ((v1, v2, v3, v4, v5, v6, v7) : CST.object_pattern) =
  let v1 = map_type_name env v1 in
  let v2 =
    (match v2 with
    | Some x -> R.Option (Some (
  ...
  in
  let v3 = (* "(" *) token env v3 in
  let v4 = map_pattern_field env v4 in
  let v5 =
    R.List (List.map (fun (v1, v2) ->
      let v1 = (* "," *) token env v1 in
...
and map_on_part (env : env) (x : CST.on_part) =
  (match x with
  | `Catch_clause_blk (v1, v2) -> R.Case ("Catch_clause_blk",
      let v1 = map_catch_clause env v1 in
      let v2 = map_block env v2 in
      R.Tuple [v1; v2]
    )
  | `On_type_not_void_opt_catch_clause_blk (v1, v2, v3, v4) -> R.Case ("On_type_not_void_opt_catch_clause_blk",
...
and map_optional_formal_parameters (env : env) (x : CST.optional_formal_parameters) =
  (match x with
  | `Opt_post_formal_params (v1, v2, v3, v4, v5) -> R.Case ("Opt_post_formal_params",
      let v1 = (* "[" *) token env v1 in
  ...
  | `Named_formal_params (v1, v2, v3, v4, v5) -> R.Case ("Named_formal_params",
      let v1 = (* "{" *) token env v1 in
...
and map_optional_parameter_types (env : env) (x : CST.optional_parameter_types) =
  (match x with
  | `Opt_posi_param_types (v1, v2, v3, v4, v5) -> R.Case ("Opt_posi_param_types",
      let v1 = (* "[" *) token env v1 in
  ...
  | `Named_param_types (v1, v2, v3, v4, v5) -> R.Case ("Named_param_types",
      let v1 = (* "{" *) token env v1 in
...
and map_outer_pattern (env : env) (x : CST.outer_pattern) =
  ...
  | `Paren_pat x -> R.Case ("Paren_pat",
      map_parenthesized_pattern env x
    )
  | `List_pat x -> R.Case ("List_pat",
      map_list_pattern env x
    )
  | `Map_pat x -> R.Case ("Map_pat",
      map_map_pattern env x
    )
...
and map_parameter_type_list (env : env) ((v1, v2, v3) : CST.parameter_type_list) =
  ...
  let v2 =
    ...
    | Some x -> R.Option (Some (
        (match x with
        | `Normal_param_type_rep_COMMA_normal_param_type_opt_COMMA (v1, v2, v3) -> R.Case ("Normal_param_type_rep_COMMA_normal_param_type_opt_COMMA",
        ...
        | `Normal_param_type_rep_COMMA_normal_param_type_COMMA_opt_param_types (v1, v2, v3, v4) -> R.Case ("Normal_param_type_rep_COMMA_normal_param_type_COMMA_opt_param_types",
            let v1 = map_normal_parameter_type env v1 in
            let v2 =
              R.List (List.map (fun (v1, v2) ->
                let v1 = (* "," *) token env v1 in
        ...
        | `Opt_param_types x -> R.Case ("Opt_param_types",
...
and map_parenthesized_pattern (env : env) ((v1, v2, v3) : CST.parenthesized_pattern) =
  let v1 = (* "(" *) token env v1 in
...
and map_pattern (env : env) (x : CST.pattern) =
  (match x with
  | `Logi_or_pat (v1, v2) -> R.Case ("Logi_or_pat",
      let v1 = map_logical_and_pattern env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "||" *) token env v1 in
          let v2 = map_logical_and_pattern env v2 in
          R.Tuple [v1; v2]
        ) v2)
      in
      R.Tuple [v1; v2]
...
and map_pattern_assignment (env : env) ((v1, v2, v3) : CST.pattern_assignment) =
  let v1 = map_outer_pattern env v1 in
  let v2 = (* "=" *) token env v2 in
  let v3 = map_argument env v3 in
  R.Tuple [v1; v2; v3]

and map_pattern_field (env : env) ((v1, v2) : CST.pattern_field) =
  let v1 =
    (match v1 with
    | Some (v1, v2) -> R.Option (Some (
        let v1 =
...
and map_pattern_variable_declaration (env : env) ((v1, v2, v3, v4) : CST.pattern_variable_declaration) =
  let v1 = map_anon_choice_final_buil_80e16d2 env v1 in
  let v2 = map_outer_pattern env v2 in
  let v3 = (* "=" *) token env v3 in
  let v4 = map_argument env v4 in
...
and map_postfix_expression (env : env) (x : CST.postfix_expression) =
  (match x with
  | `Prim_rep_sele (v1, v2) -> R.Case ("Prim_rep_sele",
      let v1 = map_primary env v1 in
      let v2 = R.List (List.map (map_selector env) v2) in
      R.Tuple [v1; v2]
    )
  | `Post_exp_ x -> R.Case ("Post_exp_",
      map_postfix_expression_ env x
    )
  )

and map_postfix_expression_ (env : env) (x : CST.postfix_expression_) =
  (match x with
  | `Assi_exp_post_op (v1, v2) -> R.Case ("Assi_exp_post_op",
      let v1 = map_assignable_expression env v1 in
      let v2 = (* increment_operator *) token env v2 in
      R.Tuple [v1; v2]
    )
  | `Cons_invo_rep_sele (v1, v2) -> R.Case ("Cons_invo_rep_sele",
      let v1 = map_constructor_invocation env v1 in
      let v2 = R.List (List.map (map_selector env) v2) in
      R.Tuple [v1; v2]
    )
...
and map_primary (env : env) (x : CST.primary) =
  ...
  | `New_exp (v1, v2, v3, v4) -> R.Case ("New_exp",
  ...
  | `Cons_tear (v1, v2, v3, v4) -> R.Case ("Cons_tear",
      let v1 = map_type_name env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
  ...
  | `Switch_exp (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("Switch_exp",
      let v1 = (* "switch" *) token env v1 in
      let v2 = map_parenthesized_expression env v2 in
      let v3 = (* "{" *) token env v3 in
      let v4 = map_switch_expression_case env v4 in
      let v5 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "," *) token env v1 in
          let v2 = map_switch_expression_case env v2 in
          R.Tuple [v1; v2]
        ) v5)
      in
      let v6 =
...
and map_primary_pattern (env : env) (x : CST.primary_pattern) =
  ...
  | `Var_pat (v1, v2) -> R.Case ("Var_pat",
      let v1 = map_final_var_or_type env v1 in
      let v2 = (* pattern [a-zA-Z_$][\w$]* *) token env v2 in
      R.Tuple [v1; v2]
    )
  | `Paren_pat x -> R.Case ("Paren_pat",
...
and map_real_expression (env : env) (x : CST.real_expression) =
  ...
  | `Logi_or_exp (v1, v2) -> R.Case ("Logi_or_exp",
      let v1 = map_real_expression env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "||" *) token env v1 in
  ...
  | `Rela_exp x -> R.Case ("Rela_exp",
      map_relational_expression env x
    )
  | `Equa_exp x -> R.Case ("Equa_exp",
      map_equality_expression env x
    )
  | `Logi_and_exp (v1, v2) -> R.Case ("Logi_and_exp",
      let v1 = map_real_expression env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "&&" *) token env v1 in
  ...
  | `Bitw_and_exp x -> R.Case ("Bitw_and_exp",
      map_bitwise_and_expression env x
    )
  | `Bitw_or_exp x -> R.Case ("Bitw_or_exp",
      map_bitwise_or_expression env x
    )
  | `Bitw_xor_exp x -> R.Case ("Bitw_xor_exp",
      map_bitwise_xor_expression env x
    )
  | `Shift_exp x -> R.Case ("Shift_exp",
      map_shift_expression env x
    )
  | `Type_cast_exp (v1, v2) -> R.Case ("Type_cast_exp",
      let v1 = map_real_expression env v1 in
      let v2 = map_type_cast env v2 in
      R.Tuple [v1; v2]
    )
  | `Type_test_exp (v1, v2) -> R.Case ("Type_test_exp",
      let v1 = map_real_expression env v1 in
...
and map_record_field (env : env) ((v1, v2) : CST.record_field) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_label env x
...
and map_record_type (env : env) (x : CST.record_type) =
  (match x with
  ...
  | `LPAR_record_type_field_rep_COMMA_record_type_field_COMMA_LCURL_record_type_named_field_rep_COMMA_record_type_named_field_opt_COMMA_RCURL_RPAR (v1, v2, v3, v4, v5, v6, v7, v8, v9, v10) -> R.Case ("LPAR_record_type_field_rep_COMMA_record_type_field_COMMA_LCURL_record_type_named_field_rep_COMMA_record_type_named_field_opt_COMMA_RCURL_RPAR",
      let v1 = (* "(" *) token env v1 in
      let v2 = map_record_type_field env v2 in
      let v3 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "," *) token env v1 in
          let v2 = map_record_type_field env v2 in
      ...
      let v4 = (* "," *) token env v4 in
      let v5 = (* "{" *) token env v5 in
      let v6 = map_record_type_named_field env v6 in
      let v7 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "," *) token env v1 in
          let v2 = map_record_type_named_field env v2 in
          R.Tuple [v1; v2]
        ) v7)
      in
      let v8 =
  ...
  | `LPAR_record_type_field_rep_COMMA_record_type_field_opt_COMMA_RPAR (v1, v2, v3, v4, v5) -> R.Case ("LPAR_record_type_field_rep_COMMA_record_type_field_opt_COMMA_RPAR",
      let v1 = (* "(" *) token env v1 in
      let v2 = map_record_type_field env v2 in
      let v3 =
        R.List (List.map (fun (v1, v2) ->
  ...
  | `LPAR_LCURL_record_type_named_field_rep_COMMA_record_type_named_field_opt_COMMA_RCURL_RPAR (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("LPAR_LCURL_record_type_named_field_rep_COMMA_record_type_named_field_opt_COMMA_RCURL_RPAR",
      let v1 = (* "(" *) token env v1 in
      let v2 = (* "{" *) token env v2 in
      let v3 = map_record_type_named_field env v3 in
      let v4 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = (* "," *) token env v1 in
          let v2 = map_record_type_named_field env v2 in
...
and map_record_type_field (env : env) ((v1, v2, v3) : CST.record_type_field) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_metadata env x
...
and map_record_type_named_field (env : env) ((v1, v2) : CST.record_type_named_field) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_metadata env x
    ...
    | None -> R.Option None)
  in
  let v2 = map_typed_identifier env v2 in
  R.Tuple [v1; v2]

and map_relational_expression (env : env) (x : CST.relational_expression) =
  (match x with
  | `Real_exp_rela_op_real_exp (v1, v2, v3) -> R.Case ("Real_exp_rela_op_real_exp",
      let v1 = map_real_expression env v1 in
      let v2 = map_relational_operator env v2 in
      let v3 = map_real_expression env v3 in
      R.Tuple [v1; v2; v3]
    )
  | `Super_rela_op_real_exp (v1, v2, v3) -> R.Case ("Super_rela_op_real_exp",
      let v1 = (* "super" *) token env v1 in
      let v2 = map_relational_operator env v2 in
      let v3 = map_real_expression env v3 in
      R.Tuple [v1; v2; v3]
...
and map_relational_pattern (env : env) (x : CST.relational_pattern) =
  (match x with
  | `Choice_rela_op_real_exp (v1, v2) -> R.Case ("Choice_rela_op_real_exp",
      let v1 =
        (match v1 with
        | `Rela_op x -> R.Case ("Rela_op",
            map_relational_operator env x
  ...
  | `Un_pat x -> R.Case ("Un_pat",
      map_unary_pattern env x
    )
  )
...
and map_selector (env : env) (x : CST.selector) =
  ...
  | `Assi_sele x -> R.Case ("Assi_sele",
      map_assignable_selector env x
    )
  | `Arg_part x -> R.Case ("Arg_part",
      map_argument_part env x
    )
  | `Type_args x -> R.Case ("Type_args",
      map_type_arguments env x
    )
  )
...
and map_shift_expression (env : env) (x : CST.shift_expression) =
  (match x with
  | `Real_exp_rep1_shift_op_real_exp (v1, v2) -> R.Case ("Real_exp_rep1_shift_op_real_exp",
      let v1 = map_real_expression env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2) ->
  ...
  | `Super_rep1_shift_op_real_exp (v1, v2) -> R.Case ("Super_rep1_shift_op_real_exp",
      let v1 = (* "super" *) token env v1 in
      let v2 =
        R.List (List.map (fun (v1, v2) ->
          let v1 = map_shift_operator env v1 in
...
and map_simple_formal_parameter (env : env) (x : CST.simple_formal_parameter) =
  ...
  | `Opt_cova_id (v1, v2) -> R.Case ("Opt_cova_id",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "covariant" *) token env tok
...
and map_statement (env : env) (x : CST.statement) =
  ...
  | `For_stmt (v1, v2, v3, v4) -> R.Case ("For_stmt",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "await" *) token env tok
  ...
  | `Try_stmt (v1, v2) -> R.Case ("Try_stmt",
      ...
      let v2 =
        ...
        | Some x -> R.Option (Some (
            ...
            | `Rep1_on_part_opt_fina_clause (v1, v2) -> R.Case ("Rep1_on_part_opt_fina_clause",
  ...
  | `Yield_each_stmt (v1, v2, v3, v4) -> R.Case ("Yield_each_stmt",
      let v1 = (* "yield" *) token env v1 in
      let v2 = (* "*" *) token env v2 in
      let v3 = map_argument env v3 in
      let v4 = (* semicolon *) token env v4 in
  ...
  | `Assert_stmt (v1, v2) -> R.Case ("Assert_stmt",
...
and map_strict_formal_parameter_list (env : env) (x : CST.strict_formal_parameter_list) =
  (match x with
  | `LPAR_RPAR (v1, v2) -> R.Case ("LPAR_RPAR",
      let v1 = (* "(" *) token env v1 in
      let v2 = (* ")" *) token env v2 in
  ...
  | `LPAR_normal_formal_params_opt_COMMA_RPAR (v1, v2, v3, v4) -> R.Case ("LPAR_normal_formal_params_opt_COMMA_RPAR",
      let v1 = (* "(" *) token env v1 in
  ...
  | `LPAR_normal_formal_params_COMMA_opt_formal_params_RPAR (v1, v2, v3, v4, v5) -> R.Case ("LPAR_normal_formal_params_COMMA_opt_formal_params_RPAR",
      let v1 = (* "(" *) token env v1 in
      let v2 = map_normal_formal_parameters env v2 in
      let v3 = (* "," *) token env v3 in
  ...
  | `LPAR_opt_formal_params_RPAR (v1, v2, v3) -> R.Case ("LPAR_opt_formal_params_RPAR",
      let v1 = (* "(" *) token env v1 in
...
and map_string_literal (env : env) (xs : CST.string_literal) =
  R.List (List.map (fun x ->
    (match x with
    | `Str_lit_double_quotes x -> R.Case ("Str_lit_double_quotes",
        map_string_literal_double_quotes env x
      )
    | `Str_lit_single_quotes x -> R.Case ("Str_lit_single_quotes",
        map_string_literal_single_quotes env x
      )
    | `Str_lit_double_quotes_mult x -> R.Case ("Str_lit_double_quotes_mult",
        map_string_literal_double_quotes_multiple env x
      )
    | `Str_lit_single_quotes_mult x -> R.Case ("Str_lit_single_quotes_mult",
        map_string_literal_single_quotes_multiple env x
      )
    | `Raw_str_lit_double_quotes x -> R.Case ("Raw_str_lit_double_quotes",
        map_raw_string_literal_double_quotes env x
      )
    | `Raw_str_lit_single_quotes x -> R.Case ("Raw_str_lit_single_quotes",
        map_raw_string_literal_single_quotes env x
      )
    | `Raw_str_lit_double_quotes_mult x -> R.Case ("Raw_str_lit_double_quotes_mult",
        map_raw_string_literal_double_quotes_multiple env x
      )
    | `Raw_str_lit_single_quotes_mult x -> R.Case ("Raw_str_lit_single_quotes_mult",
        map_raw_string_literal_single_quotes_multiple env x
      )
    )
  ) xs)

and map_string_literal_double_quotes (env : env) ((v1, v2, v3) : CST.string_literal_double_quotes) =
  let v1 = (* "\"" *) token env v1 in
  let v2 =
    R.List (List.map (fun x ->
      (match x with
...
and map_string_literal_double_quotes_multiple (env : env) ((v1, v2, v3) : CST.string_literal_double_quotes_multiple) =
  let v1 = (* "\"\"\"" *) token env v1 in
  let v2 =
    R.List (List.map (fun x ->
      (match x with
...
and map_string_literal_single_quotes (env : env) ((v1, v2, v3) : CST.string_literal_single_quotes) =
  let v1 = (* "'" *) token env v1 in
  let v2 =
    R.List (List.map (fun x ->
      (match x with
...
and map_string_literal_single_quotes_multiple (env : env) ((v1, v2, v3) : CST.string_literal_single_quotes_multiple) =
  let v1 = (* "'''" *) token env v1 in
  let v2 =
    R.List (List.map (fun x ->
      (match x with
...
and map_super_formal_parameter (env : env) ((v1, v2, v3, v4, v5) : CST.super_formal_parameter) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_final_const_var_or_type env x
...
and map_switch_block (env : env) ((v1, v2, v3, v4) : CST.switch_block) =
  let v1 = (* "{" *) token env v1 in
  let v2 =
    R.List (List.map (map_switch_statement_case env) v2)
  in
  let v3 =
    (match v3 with
    | Some x -> R.Option (Some (
        map_switch_statement_default env x
      ))
    | None -> R.Option None)
  in
  let v4 = (* "}" *) token env v4 in
...
and map_switch_expression_case (env : env) ((v1, v2, v3) : CST.switch_expression_case) =
  let v1 = map_guarded_pattern env v1 in
  let v2 = (* "=>" *) token env v2 in
  let v3 = map_argument env v3 in
  R.Tuple [v1; v2; v3]

and map_switch_statement_case (env : env) ((v1, v2, v3, v4, v5) : CST.switch_statement_case) =
  let v1 = R.List (List.map (map_label env) v1) in
  let v2 = (* case_builtin *) token env v2 in
  let v3 = map_guarded_pattern env v3 in
  let v4 = (* ":" *) token env v4 in
  let v5 = R.List (List.map (map_statement env) v5) in
  R.Tuple [v1; v2; v3; v4; v5]

and map_switch_statement_default (env : env) ((v1, v2, v3, v4) : CST.switch_statement_default) =
  let v1 = R.List (List.map (map_label env) v1) in
...
and map_template_substitution (env : env) ((v1, v2) : CST.template_substitution) =
  ...
  let v2 =
    ...
    | `Id_dollar_esca tok -> R.Case ("Id_dollar_esca",
        (* pattern ([a-zA-Z_]|(\\\$))([\w]|(\\\$))* *) token env tok
      )
    )
  in
...
and map_type_ (env : env) (x : CST.type_) =
  (match x with
  | `Func_type_opt_null_type (v1, v2) -> R.Case ("Func_type_opt_null_type",
  ...
  | `Type_not_func x -> R.Case ("Type_not_func",
      map_type_not_function env x
    )
  )

and map_type_arguments (env : env) (x : CST.type_arguments) =
  (match x with
  | `LT_opt_type_rep_COMMA_type_GT (v1, v2, v3) -> R.Case ("LT_opt_type_rep_COMMA_type_GT",
      let v1 = (* "<" *) token env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
            map_interface_type_list env x
          ))
        | None -> R.Option None)
      in
      let v3 = (* ">" *) token env v3 in
...
and map_type_not_function (env : env) (x : CST.type_not_function) =
  (match x with
  | `Type_not_void_not_func x -> R.Case ("Type_not_void_not_func",
      map_type_not_void_not_function env x
    )
  | `Record_type_opt_null_type (v1, v2) -> R.Case ("Record_type_opt_null_type",
      let v1 = map_record_type env v1 in
      let v2 =
        (match v2 with
        | Some tok -> R.Option (Some (
            (* "?" *) token env tok
...
and map_type_not_void_not_function (env : env) (x : CST.type_not_void_not_function) =
  (match x with
  | `Type_name_opt_type_args_opt_null_type (v1, v2, v3) -> R.Case ("Type_name_opt_type_args_opt_null_type",
      let v1 = map_type_name env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
  ...
  | `Func_buil_id_opt_null_type (v1, v2) -> R.Case ("Func_buil_id_opt_null_type",
      let v1 = (* "Function" *) token env v1 in
      let v2 =
        (match v2 with
        | Some tok -> R.Option (Some (
...
and map_type_parameter (env : env) ((v1, v2, v3, v4) : CST.type_parameter) =
  ...
  let v2 =
    ...
    | `Null_type tok -> R.Case ("Null_type",
        (* "?" *) token env tok
      )
    )
  in
...
and map_unary_expression_ (env : env) (x : CST.unary_expression_) =
  (match x with
  | `Prefix_op_un_exp (v1, v2) -> R.Case ("Prefix_op_un_exp",
  ...
  | `Choice_minus_op_super (v1, v2) -> R.Case ("Choice_minus_op_super",
      let v1 =
        (match v1 with
        | `Minus_op tok -> R.Case ("Minus_op",
            (* "-" *) token env tok
  ...
  | `Incr_op_assi_exp (v1, v2) -> R.Case ("Incr_op_assi_exp",
...
and map_unary_pattern (env : env) (x : CST.unary_pattern) =
  (match x with
  | `Cast_pat (v1, v2, v3) -> R.Case ("Cast_pat",
      let v1 = map_primary_pattern env v1 in
      let v2 = (* "as" *) token env v2 in
      let v3 = map_type_ env v3 in
      R.Tuple [v1; v2; v3]
    )
  | `Null_check_pat (v1, v2) -> R.Case ("Null_check_pat",
      let v1 = map_primary_pattern env v1 in
      let v2 = (* "?" *) token env v2 in
      R.Tuple [v1; v2]
    )
  | `Null_assert_pat (v1, v2) -> R.Case ("Null_assert_pat",
      let v1 = map_primary_pattern env v1 in
      let v2 = (* "!" *) token env v2 in
      R.Tuple [v1; v2]
    )
  | `Prim_pat x -> R.Case ("Prim_pat",
      map_primary_pattern env x
    )
  )

and map_unconditional_assignable_selector (env : env) (x : CST.unconditional_assignable_selector) =
  (match x with
  | `Index_sele x -> R.Case ("Index_sele",
      map_index_selector env x
    )
  | `DOT_id x -> R.Case ("DOT_id",
      map_dot_identifier env x
    )
  )
...
and map_var_or_type (env : env) (x : CST.var_or_type) =
  ...
  | `Infe_type_opt_type (v1, v2) -> R.Case ("Infe_type_opt_type",
...
let map_explicit_constructor_invocation (env : env) ((v1, v2, v3) : CST.explicit_constructor_invocation) =
  let v1 =
    (match v1 with
    ...
    | `Choice_choice_id_DOT_opt_type_args_super (v1, v2, v3, v4) -> R.Case ("Choice_choice_id_DOT_opt_type_args_super",
        let v1 =
          (match v1 with
          | `Choice_id x -> R.Case ("Choice_id",
              map_ambiguous_name env x
            )
          | `Prim x -> R.Case ("Prim",
              map_primary env x
            )
...
let map_operator_signature (env : env) ((v1, v2, v3, v4, v5) : CST.operator_signature) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_type_ env x
...
let map_part_of_directive (env : env) ((v1, v2, v3, v4) : CST.part_of_directive) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_metadata env x
    ...
    | None -> R.Option None)
  in
  let v2 = (* part_of_builtin *) token env v2 in
  let v3 =
    (match v3 with
    | `Dotted_id_list x -> R.Case ("Dotted_id_list",
...
let map_factory_constructor_signature (env : env) ((v1, v2, v3, v4) : CST.factory_constructor_signature) =
...
let map_enum_constant (env : env) (x : CST.enum_constant) =
  (match x with
  | `Opt_meta_id_opt_arg_part (v1, v2, v3) -> R.Case ("Opt_meta_id_opt_arg_part",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
  ...
  | `Opt_meta_id_opt_type_args_DOT_choice_id_args (v1, v2, v3, v4, v5, v6) -> R.Case ("Opt_meta_id_opt_type_args_DOT_choice_id_args",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
      ...
      let v4 = (* "." *) token env v4 in
      let v5 = map_identifier_or_new env v5 in
      let v6 = map_arguments env v6 in
...
let map_type_alias (env : env) (x : CST.type_alias) =
  (match x with
  | `Opt_meta_type_opt_type_type_name_formal_param_part_SEMI (v1, v2, v3, v4, v5, v6) -> R.Case ("Opt_meta_type_opt_type_type_name_formal_param_part_SEMI",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
      ...
      let v5 = map_formal_parameter_part env v5 in
      let v6 = (* ";" *) token env v6 in
      R.Tuple [v1; v2; v3; v4; v5; v6]
    )
  | `Opt_meta_type_type_name_opt_type_params_EQ_type_SEMI (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("Opt_meta_type_type_name_opt_type_params_EQ_type_SEMI",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
...
let map_wildcard_bounds (env : env) (x : CST.wildcard_bounds) =
  (match x with
  | `Extends_type (v1, v2) -> R.Case ("Extends_type",
      let v1 = (* "extends" *) token env v1 in
      let v2 = map_type_ env v2 in
      R.Tuple [v1; v2]
    )
...
let map_setter_signature (env : env) ((v1, v2, v3, v4, v5) : CST.setter_signature) =
  let v1 =
    (match v1 with
    | Some x -> R.Option (Some (
        map_type_ env x
...
let map_static_final_declaration (env : env) ((v1, v2, v3) : CST.static_final_declaration) =
  let v1 = (* pattern [a-zA-Z_$][\w$]* *) token env v1 in
  let v2 = (* "=" *) token env v2 in
  let v3 = map_argument env v3 in
  R.Tuple [v1; v2; v3]

let map_constructor_signature (env : env) ((v1, v2, v3) : CST.constructor_signature) =
  let v1 = (* pattern [a-zA-Z_$][\w$]* *) token env v1 in
  let v2 =
    (match v2 with
    | Some (v1, v2) -> R.Option (Some (
...
let map_initialized_identifier_list (env : env) ((v1, v2) : CST.initialized_identifier_list) =
...
let map_constant_constructor_signature (env : env) ((v1, v2, v3, v4) : CST.constant_constructor_signature) =
  let v1 = (* const_builtin *) token env v1 in
  let v2 = (* pattern [a-zA-Z_$][\w$]* *) token env v2 in
  let v3 =
    (match v3 with
...
let map_configuration_uri_condition (env : env) ((v1, v2, v3) : CST.configuration_uri_condition) =
  let v1 = (* "(" *) token env v1 in
  let v2 = map_uri_test env v2 in
  let v3 = (* ")" *) token env v3 in
  R.Tuple [v1; v2; v3]

let map_static_final_declaration_list (env : env) ((v1, v2) : CST.static_final_declaration_list) =
...
let map_initializer_list_entry (env : env) (x : CST.initializer_list_entry) =
  (match x with
  | `Super_args (v1, v2) -> R.Case ("Super_args",
      let v1 = (* "super" *) token env v1 in
      let v2 = map_arguments env v2 in
      R.Tuple [v1; v2]
    )
  | `Super_DOT_choice_id_args (v1, v2, v3, v4) -> R.Case ("Super_DOT_choice_id_args",
      let v1 = (* "super" *) token env v1 in
      let v2 = (* "." *) token env v2 in
      let v3 = map_identifier_or_new env v3 in
      let v4 = map_arguments env v4 in
...
let map_superclass (env : env) (x : CST.superclass) =
  (match x with
  | `Extends_type_not_void_opt_mixins (v1, v2, v3) -> R.Case ("Extends_type_not_void_opt_mixins",
      let v1 = (* "extends" *) token env v1 in
      let v2 = map_type_not_void env v2 in
      let v3 =
        (match v3 with
  ...
  | `Mixins x -> R.Case ("Mixins",
      map_mixins env x
    )
  )

let map_configuration_uri (env : env) ((v1, v2, v3) : CST.configuration_uri) =
  let v1 = (* "if" *) token env v1 in
  let v2 = map_configuration_uri_condition env v2 in
  let v3 = map_uri env v3 in
  R.Tuple [v1; v2; v3]
...
let map_mixin_application_class (env : env) ((v1, v2, v3, v4, v5) : CST.mixin_application_class) =
  let v1 = (* pattern [a-zA-Z_$][\w$]* *) token env v1 in
  let v2 =
    (match v2 with
    | Some x -> R.Option (Some (
...
let map_method_signature (env : env) (x : CST.method_signature) =
  (match x with
  | `Cons_sign_opt_initis (v1, v2) -> R.Case ("Cons_sign_opt_initis",
      let v1 = map_constructor_signature env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
            map_initializers env x
          ))
        | None -> R.Option None)
      in
      R.Tuple [v1; v2]
    )
  | `Fact_cons_sign x -> R.Case ("Fact_cons_sign",
      map_factory_constructor_signature env x
    )
  | `Opt_static_choice_func_sign (v1, v2) -> R.Case ("Opt_static_choice_func_sign",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "static" *) token env tok
      ...
      let v2 =
        ...
        | `Getter_sign x -> R.Case ("Getter_sign",
            map_getter_signature env x
          )
        | `Setter_sign x -> R.Case ("Setter_sign",
            map_setter_signature env x
          )
        )
      in
      R.Tuple [v1; v2]
    )
  | `Op_sign x -> R.Case ("Op_sign",
      map_operator_signature env x
    )
  )

let map_anon_choice_redi_3f8cf96 (env : env) (x : CST.anon_choice_redi_3f8cf96) =
  (match x with
  | `Redi (v1, v2, v3, v4) -> R.Case ("Redi",
      let v1 = (* ":" *) token env v1 in
      let v2 = (* "this" *) token env v2 in
      let v3 =
        (match v3 with
...
let map_import_specification (env : env) (x : CST.import_specification) =
  (match x with
  | `Import_conf_uri_opt_as_id_rep_comb_semi (v1, v2, v3, v4, v5) -> R.Case ("Import_conf_uri_opt_as_id_rep_comb_semi",
  ...
  | `Import_uri_defe_as_id_rep_comb_semi (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("Import_uri_defe_as_id_rep_comb_semi",
      let v1 = (* "import" *) token env v1 in
      let v2 = map_uri env v2 in
      ...
      let v7 = (* semicolon *) token env v7 in
      R.Tuple [v1; v2; v3; v4; v5; v6; v7]
    )
  )
...
let map_declaration_ (env : env) (x : CST.declaration_) =
  (match x with
  | `Cst_cons_sign_opt_choice_redi (v1, v2) -> R.Case ("Cst_cons_sign_opt_choice_redi",
      let v1 = map_constant_constructor_signature env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
            map_anon_choice_redi_3f8cf96 env x
          ))
        | None -> R.Option None)
      in
      R.Tuple [v1; v2]
    )
  | `Cons_sign_opt_choice_redi (v1, v2) -> R.Case ("Cons_sign_opt_choice_redi",
      let v1 = map_constructor_signature env v1 in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
  ...
  | `Exte_opt_const_buil_fact_cons_sign (v1, v2, v3) -> R.Case ("Exte_opt_const_buil_fact_cons_sign",
      let v1 = (* "external" *) token env v1 in
      let v2 =
        (match v2 with
        | Some tok -> R.Option (Some (
  ...
  | `Opt_const_buil_fact_cons_sign_native (v1, v2, v3) -> R.Case ("Opt_const_buil_fact_cons_sign_native",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* const_builtin *) token env tok
  ...
  | `Exte_cst_cons_sign (v1, v2) -> R.Case ("Exte_cst_cons_sign",
      let v1 = (* "external" *) token env v1 in
      let v2 = map_constant_constructor_signature env v2 in
      R.Tuple [v1; v2]
    )
  | `Redi_fact_cons_sign (v1, v2, v3, v4, v5, v6, v7, v8) -> R.Case ("Redi_fact_cons_sign",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* const_builtin *) token env tok
  ...
  | `Exte_cons_sign (v1, v2) -> R.Case ("Exte_cons_sign",
      let v1 = (* "external" *) token env v1 in
      let v2 = map_constructor_signature env v2 in
      R.Tuple [v1; v2]
    )
  | `Opt_exte_buil_opt_static_getter_sign (v1, v2, v3) -> R.Case ("Opt_exte_buil_opt_static_getter_sign",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
  ...
  | `Opt_exte_and_static_setter_sign (v1, v2) -> R.Case ("Opt_exte_and_static_setter_sign",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_external_and_static env x
          ))
        | None -> R.Option None)
      in
  ...
  | `Opt_exte_op_sign (v1, v2) -> R.Case ("Opt_exte_op_sign",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "external" *) token env tok
  ...
  | `Opt_exte_and_static_func_sign (v1, v2) -> R.Case ("Opt_exte_and_static_func_sign",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_external_and_static env x
  ...
  | `Static_choice_final_or_const_opt_type_static_final_decl_list (v1, v2) -> R.Case ("Static_choice_final_or_const_opt_type_static_final_decl_list",
      let v1 = (* "static" *) token env v1 in
      let v2 =
        (match v2 with
        | `Final_or_const_opt_type_static_final_decl_list (v1, v2, v3) -> R.Case ("Final_or_const_opt_type_static_final_decl_list",
        ...
        | `Late_buil_choice_final_buil_opt_type_init_id_list (v1, v2) -> R.Case ("Late_buil_choice_final_buil_opt_type_init_id_list",
            let v1 = (* "late" *) token env v1 in
            let v2 =
              (match v2 with
              | `Final_buil_opt_type_init_id_list (v1, v2, v3) -> R.Case ("Final_buil_opt_type_init_id_list",
  ...
  | `Opt_late_buil_final_buil_opt_type_init_id_list (v1, v2, v3, v4) -> R.Case ("Opt_late_buil_final_buil_opt_type_init_id_list",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "late" *) token env tok
  ...
  | `Opt_late_buil_var_or_type_init_id_list (v1, v2, v3) -> R.Case ("Opt_late_buil_var_or_type_init_id_list",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "late" *) token env tok
...
let map_extension_body (env : env) ((v1, v2, v3) : CST.extension_body) =
  ...
  let v2 =
    R.List (List.map (fun x ->
      (match x with
      | `Opt_meta_decl__semi (v1, v2, v3) -> R.Case ("Opt_meta_decl__semi",
          let v1 =
            (match v1 with
            | Some x -> R.Option (Some (
                map_metadata env x
      ...
      | `Opt_meta_meth_sign_func_body (v1, v2, v3) -> R.Case ("Opt_meta_meth_sign_func_body",
          let v1 =
            (match v1 with
            | Some x -> R.Option (Some (
                map_metadata env x
...
let map_import_or_export (env : env) (x : CST.import_or_export) =
  (match x with
  | `Libr_import (v1, v2) -> R.Case ("Libr_import",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
  ...
  | `Libr_export (v1, v2, v3, v4, v5) -> R.Case ("Libr_export",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
...
let map_anon_rep_opt_meta_class_member_defi_cd2fbdb (env : env) (xs : CST.anon_rep_opt_meta_class_member_defi_cd2fbdb) =
  R.List (List.map (fun (v1, v2) ->
    let v1 =
      (match v1 with
      | Some x -> R.Option (Some (
...
let map_extension_declaration (env : env) (x : CST.extension_declaration) =
  (match x with
  | `Opt_meta_exte_opt_id_opt_type_params_on_type_exte_body (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("Opt_meta_exte_opt_id_opt_type_params_on_type_exte_body",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
        ...
        | None -> R.Option None)
      in
      let v2 = (* "extension" *) token env v2 in
      let v3 =
        (match v3 with
        | Some tok -> R.Option (Some (
            (* pattern [a-zA-Z_$][\w$]* *) token env tok
      ...
      let v4 =
        ...
        | None -> R.Option None)
      in
      let v5 = (* "on" *) token env v5 in
      let v6 = map_type_ env v6 in
      let v7 = map_extension_body env v7 in
      R.Tuple [v1; v2; v3; v4; v5; v6; v7]
    )
  )

let map_enum_body (env : env) ((v1, v2, v3, v4, v5, v6) : CST.enum_body) =
  let v1 = (* "{" *) token env v1 in
  ...
  let v5 =
    ...
    | Some (v1, v2) -> R.Option (Some (
        let v1 = (* ";" *) token env v1 in
        let v2 =
          map_anon_rep_opt_meta_class_member_defi_cd2fbdb env v2
        in
        R.Tuple [v1; v2]
      ))
    | None -> R.Option None)
...
let map_class_definition (env : env) (x : CST.class_definition) =
  ...
  | `Opt_meta_choice_class_modifs_id_opt_type_params_opt_supe_opt_inters_class_body (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("Opt_meta_choice_class_modifs_id_opt_type_params_opt_supe_opt_inters_class_body",
      ...
      let v2 =
        (match v2 with
        | `Class_modifs x -> R.Case ("Class_modifs",
            map_class_modifiers env x
          )
        | `Mixin_class_modifs x -> R.Case ("Mixin_class_modifs",
            map_mixin_class_modifiers env x
          )
        )
      in
      let v3 = (* pattern [a-zA-Z_$][\w$]* *) token env v3 in
  ...
  | `Opt_meta_class_modifs_mixin_app_class (v1, v2, v3) -> R.Case ("Opt_meta_class_modifs_mixin_app_class",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_metadata env x
        ...
        | None -> R.Option None)
      in
      let v2 = map_class_modifiers env v2 in
      let v3 = map_mixin_application_class env v3 in
      R.Tuple [v1; v2; v3]
    )
  )
...
let map_program (env : env) (x : CST.program) =
  (match x with
  | `Opt_script_tag_opt_libr_name_rep_import_or_export_rep_part_dire_rep_part_of_dire_rep_top_level_defi (v1, v2, v3, v4, v5, v6) -> R.Case ("Opt_script_tag_opt_libr_name_rep_import_or_export_rep_part_dire_rep_part_of_dire_rep_top_level_defi",
      let v1 =
        (match v1 with
        | Some x -> R.Option (Some (
            map_script_tag env x
          ))
        | None -> R.Option None)
      in
      let v2 =
      ...
      let v6 =
        R.List (List.map (map_top_level_definition env) v6)
      in
      R.Tuple [v1; v2; v3; v4; v5; v6]
    )
