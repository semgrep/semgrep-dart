...
let map_final_or_const (env : env) (x : CST.final_or_const) =
  (match x with
  | `Final_buil tok -> (* final_builtin *) token env tok
  | `Const_buil tok -> (* const_builtin *) token env tok
  )
...
let map_dotted_identifier_list (env : env) ((v1, v2) : CST.dotted_identifier_list) =
  let v1 = (* pattern [a-zA-Z_$][\w$]* *) token env v1 in
  let v2 = List.map (map_dot_identifier env) v2 in
  todo env (v1, v2)
...
let map_binary_operator (env : env) (x : CST.binary_operator) =
  ...
  | `Addi_op tok -> (* additive_operator_ *) token env tok
...
and map_assignable_selector_part (env : env) ((v1, v2) : CST.assignable_selector_part) =
  let v1 = List.map (map_selector env) v1 in
...
and map_expression (env : env) (x : CST.expression) =
  ...
  | `Real_exp_rep_casc_sect (v1, v2) ->
      let v1 = map_real_expression env v1 in
      let v2 = List.map (map_cascade_section env) v2 in
      todo env (v1, v2)
  )
...
and map_for_loop_parts (env : env) (x : CST.for_loop_parts) =
  ...
  | `Choice_decl_id_in_exp (v1, v2, v3) ->
      ...
      let v3 = map_argument env v3 in
      todo env (v1, v2, v3)
  | `Opt_choice_local_var_decl_opt_exp_semi_opt_exp_rep_COMMA_exp (v1, v2, v3, v4) ->
      let v1 =
        (match v1 with
        | Some x ->
            (match x with
...
and map_function_type_tails (env : env) (xs : CST.function_type_tails) =
  List.map (map_function_type_tail env) xs
...
and map_literal (env : env) (x : CST.literal) =
  ...
  | `List_lit (v1, v2, v3, v4, v5) ->
      let v1 =
        (match v1 with
        | Some tok -> (* const_builtin *) token env tok
        | None -> todo env ())
      in
      let v2 =
        (match v2 with
  ...
  let v2 =
    ...
    | Some x ->
        ...
        | `Normal_param_type_rep_COMMA_normal_param_type_opt_COMMA (v1, v2, v3) ->
            ...
            in
            todo env (v1, v2, v3)
        | `Normal_param_type_rep_COMMA_normal_param_type_COMMA_opt_param_types (v1, v2, v3, v4) ->
            let v1 = map_normal_parameter_type env v1 in
            let v2 =
              List.map (fun (v1, v2) ->
                let v1 = (* "," *) token env v1 in
...
and map_postfix_expression (env : env) (x : CST.postfix_expression) =
  ...
  | `Prim_rep_sele (v1, v2) ->
      let v1 = map_primary env v1 in
      let v2 = List.map (map_selector env) v2 in
      todo env (v1, v2)
...
and map_statement (env : env) (x : CST.statement) =
  ...
  | `Brk_stmt (v1, v2, v3) ->
      ...
      let v2 =
        (match v2 with
        | Some tok -> (* pattern [a-zA-Z_$][\w$]* *) token env tok
        | None -> todo env ())
      in
      let v3 = map_semicolon env v3 in
      todo env (v1, v2, v3)
...
and map_switch_label (env : env) ((v1, v2) : CST.switch_label) =
  let v1 = List.map (map_label env) v1 in
  let v2 =
    (match v2 with
...
and map_type_not_function (env : env) (x : CST.type_not_function) =
  ...
  | `Void_type tok -> (* void_type *) token env tok
  )
...
let map_explicit_constructor_invocation (env : env) ((v1, v2, v3) : CST.explicit_constructor_invocation) =
  let v1 =
    ...
    | `Choice_choice_id_DOT_opt_type_args_super (v1, v2, v3, v4) ->
        let v1 =
          (match v1 with
          | `Choice_id x -> map_ambiguous_name env x
...
let map_anon_choice_type_be0da33 (env : env) (x : CST.anon_choice_type_be0da33) =
  (match x with
  | `Type x -> map_type_ env x
  | `Infe_type tok -> (* "var" *) token env tok
  )
...
let map_factory_constructor_signature (env : env) ((v1, v2, v3, v4) : CST.factory_constructor_signature) =
  ...
  let v2 = (* pattern [a-zA-Z_$][\w$]* *) token env v2 in
  let v3 = List.map (map_dot_identifier env) v3 in
  let v4 = map_formal_parameter_list env v4 in
  todo env (v1, v2, v3, v4)
...
let map_configurable_uri (env : env) ((v1, v2) : CST.configurable_uri) =
  ...
  let v2 = List.map (map_configuration_uri env) v2 in
  todo env (v1, v2)
...
let map_import_specification (env : env) (x : CST.import_specification) =
  ...
  | `Import_conf_uri_opt_as_id_rep_comb_semi (v1, v2, v3, v4, v5) ->
      ...
      let v3 =
        ...
        | None -> todo env ())
      in
      let v4 = List.map (map_combinator env) v4 in
      let v5 = map_semicolon env v5 in
      todo env (v1, v2, v3, v4, v5)
  | `Import_uri_defe_as_id_rep_comb_semi (v1, v2, v3, v4, v5, v6, v7) ->
      ...
      let v6 = List.map (map_combinator env) v6 in
...
let map_declaration_ (env : env) (x : CST.declaration_) =
  ...
  | `Redi_fact_cons_sign (v1, v2, v3, v4, v5, v6, v7, v8) ->
      ...
      let v3 = (* pattern [a-zA-Z_$][\w$]* *) token env v3 in
      let v4 = List.map (map_dot_identifier env) v4 in
...
let map_class_definition (env : env) (x : CST.class_definition) =
  (match x with
  | `Opt_abst_class_id_opt_type_params_opt_supe_opt_inters_class_body (v1, v2, v3, v4, v5, v6, v7) ->
      let v1 =
        (match v1 with
...
let map_program (env : env) ((v1, v2, v3, v4, v5, v6, v7) : CST.program) =
  ...
  let v2 =
    ...
    | None -> todo env ())
  in
  let v3 = List.map (map_import_or_export env) v3 in
  let v4 = List.map (map_part_directive env) v4 in
  let v5 = List.map (map_part_of_directive env) v5 in
  let v6 =
    List.map (fun (v1, v2) ->
      let v1 =
        (match v1 with
  ...
  in
  let v7 = List.map (map_statement env) v7 in
  todo env (v1, v2, v3, v4, v5, v6, v7)
